...
namespace DynMvp.Devices.FrameGrabber
{
    ...
    public class ImageProvider
    {
        ...
        public PYLON_DEVICE_HANDLE DeviceHandle
        {
        ...
        protected PYLON_WAITOBJECT_HANDLE m_hStop;         /* Handle used for waiting for a grab to be finished. */
        PYLON_CHUNKPARSER_HANDLE m_hChunkParser;
        ...
        public ImageProvider()
        {
            ...
            m_hStop = PylonC.NET.Pylon.WaitObjectCreate();
        }
        ...
        public void Close()
        {
            ...
            PylonC.NET.Pylon.DeviceDestroyChunkParser(m_hDevice, m_hChunkParser);
        ...
        public void OneShot()
        {
            ...
            {
                PylonC.NET.Pylon.WaitObjectReset(m_hStop);
        ...
        public void Stop()
        {
            ...
            {
                PylonC.NET.Pylon.WaitObjectSignal(m_hStop);                

                m_grabThreadRun = false; /* Causes the grab thread to stop. */
                m_grabThread.Join(1000); /* Wait for it to stop. */
            }
        }
        ...
        protected void Open(PYLON_DEVICE_HANDLE device)
        {
            try
            {
                ...
                m_hChunkParser = PylonC.NET.Pylon.DeviceCreateChunkParser(m_hDevice);
                ...
                //PylonC.NET.Pylon.DeviceFeatureFromString(m_hDevice, "PixelFormat", "Mono8");
        ...
        protected void Grab()
        {
            try
            {
                ...
                PYLON_WAITOBJECTS_HANDLE waitObjects = PylonC.NET.Pylon.WaitObjectsCreate();
                PylonC.NET.Pylon.WaitObjectsAdd(waitObjects, m_hWait);
                PylonC.NET.Pylon.WaitObjectsAdd(waitObjects, m_hStop);
                ...
                while (m_grabThreadRun) /* Is set to false when stopping to end the grab thread. */
                {
                    ...
                    uint index = 0;
                    if (!PylonC.NET.Pylon.WaitObjectsWaitForAny(waitObjects, 20000, out index))
                    {
                        lock (m_lockObject)
                        {
        ...
        protected void EnqueueTakenImage(PylonGrabResult_t grabResult)
        {
            ...
            var bufferIndex = (int)grabResult.Context;
            ...
            if (grabResult.PayloadType == EPylonPayloadType.PayloadType_ChunkData)
            {
                try
                {
                    if (m_hChunkParser.IsValid)
                    {
                        PylonC.NET.Pylon.ChunkParserAttachBuffer(m_hChunkParser, buffer);

                        /* Retrieve the chunk width value. */
                        /* ... Check the availability. */

                        isAvail = PylonC.NET.Pylon.DeviceFeatureIsAvailable(m_hDevice, "ChunkWidth");
                        if (isAvail)
                        {
                            /* ... Get the value. */
                            chunkWidth = PylonC.NET.Pylon.DeviceGetIntegerFeature(m_hDevice, "ChunkWidth");
                        }

                        /* Retrieve the chunk height value. */
                        /* ... Check the availability. */
                        isAvail = PylonC.NET.Pylon.DeviceFeatureIsAvailable(m_hDevice, "ChunkHeight");
                        if (isAvail)
                        {
                            /* ... Get the value. */
                            chunkHeight = PylonC.NET.Pylon.DeviceGetIntegerFeature(m_hDevice, "ChunkHeight");

                        }
                        Debug.WriteLine("Image Grabbed sizeX= {0},sizeY={1} Width = {2}, Height= {3}", grabResult.SizeX, grabResult.SizeY, chunkWidth, chunkHeight);
                        /* Before requeueing the buffer, you should detach it from the chunk parser. */
                        isAvail = PylonC.NET.Pylon.DeviceFeatureIsAvailable(m_hDevice, "ChunkPixelFormat");
                        if (isAvail)
                        {
                            /* ... Get the value. */
                            var format = PylonC.NET.Pylon.DeviceFeatureToString(m_hDevice, "ChunkPixelFormat");

                        }
                        Debug.WriteLine("Image Grabbed sizeX= {0},sizeY={1} Width = {2}, Height= {3}", grabResult.SizeX, grabResult.SizeY, chunkWidth, chunkHeight);
                        PylonC.NET.Pylon.ChunkParserDetachBuffer(m_hChunkParser);  /* Now the chunk data in the buffer is no longer accessible. */
                    }
                }
                catch (Exception ex)
                {
                    Debug.WriteLine("Pylon Exception - {0}", ex.Message);
                    throw ex;
                }

                newGrabResultInternal.ImageData = new Image((int)chunkWidth, (int)chunkHeight, buffer.Array, false); //mono ¿œ∂ß∏∏
            }
            else if(grabResult.PayloadType == EPylonPayloadType.PayloadType_Image)
            {
                ...
                {
                    newGrabResultInternal.ImageData = new Image(grabResult.SizeX, grabResult.SizeY, buffer.Array, grabResult.PixelType == EPylonPixelType.PixelType_BGR8packed);
                }
                else /* Conversion is required. */
                {
                    ...
                    PylonC.NET.Pylon.ImageFormatConverterSetOutputPixelFormat(m_hConverter, m_converterOutputFormatIsColor ? EPylonPixelType.PixelType_BGR8packed : EPylonPixelType.PixelType_Mono8);
