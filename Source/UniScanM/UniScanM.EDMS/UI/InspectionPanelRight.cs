    ...
    public partial class InspectionPanelRight : UserControl, IInspectionPanel, IMultiLanguageSupport, IOpStateListener
    {
        ...
        public void AsyncDraw()
        {
            ...
            lock (lockobjRevison)
            {
                ...
                TextFigure text = new TextFigure(nDisplayRevisionCount.ToString(), new Point(200, 200), new Font(FontFamily.GenericSerif, 200), Color.Yellow); ;
                figureGroup.AddFigure(text);
                //화면에 프레임 intensity 표시

                TextFigure text2 = new TextFigure(myInspectionResult.FrameAvgIntensity.ToString("#.0"), new Point(200, 500), new Font(FontFamily.GenericSerif, 200), Color.Red); ;
        ...
        private void DrawReferenceLine(FigureGroup figureGroup, Bitmap bitmap, Data.InspectionResult myInspectionResult)
        {
            ...
            {
                verticalOriginStartPt = new PointF(3 * 1000 / 5, 0);
                verticalOriginEndPt = new PointF(3 * 1000 / 5, bitmap.Height);
            }
        ...
        private void DrawEdgeLine(FigureGroup figureGroup, Bitmap bitmap, Data.InspectionResult myInspectionResult)
        {
            ...
            if (pixelPosition != null && bitmap != null)
            {
                ...
                ///////////////////////////////////
                ///Profile ////////////////////////
                float[] profile = myInspectionResult.Profile;
                if (profile != null && !checkOnTune.Checked)
                {
                    Pen penProfile = new Pen(Color.FromArgb(90, 0, 200, 0), 1);
                    ...
                    {
                        y = bitmap.Height - (int)(profile[x] * Scale) - 1;
                    ...
                    for (int i = 0; i < 3; i++)
                    {
                        int sX = (int)(pixelPosition[i] - linewidth);
                        int eX = (int)(pixelPosition[i] + linewidth);
                        if (EDMSSettings.Instance().IsFrontPosition)
                        {
                            sX = (int)(bitmap.Width - pixelPosition[i] - linewidth);
                            eX = (int)(bitmap.Width - pixelPosition[i] + linewidth);
                        }
        ...
        private void lightValue_ValueChanged(object sender, EventArgs e)
        {
            ...
            if (currentModel != null)
            {
                ...
                int[] lightvalue = new int[lightCount];
                lightvalue[0] = (int)backLightValue.Value; //backLight

                // 이동형은 상부조명이 2개이므로 ...
                for (int i=1; i<lightCount; i++)
                    lightvalue[i] = (int)frontLightValue.Value; 
