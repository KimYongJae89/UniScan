...
namespace UniScanM.EDMS
{
    static class Program
    {
            ...
            if (bNew)
            {
                ...
                try
                {
                    ...
                    if (SystemManager.Instance().Setup() == true)
                    {
                        ...
                        if (Settings.EDMSSettings.Instance().ClearProductionValueData)
                        {
                            SystemManager.Instance().ProductionManager.List.ForEach(f => ((UniScanM.Data.Production)f).Value = float.NaN);
                            SystemManager.Instance().ProductionManager.Save();
                            Settings.EDMSSettings.Instance().ClearProductionValueData = false;
                            Settings.EDMSSettings.Instance().Save();
                        }
        ...
        public static void BuildSystemManager()
        {
            ...
            MachineIfProtocolList.Set(new UniScanM.EDMS.MachineIF.MachineIfProtocolList());
            systemManager.Init(
