...
namespace UniScanM.EDMSW.UI
{
    public class ReportPageController : UniScanM.UI.ReportPageController
    {
        public override void Initialize(DataGridView dataGridView)
        {
            ...
            dataGridView.Columns.Add(new DataGridViewTextBoxColumn { HeaderText = StringManager.GetString(this.GetType().FullName, "Variance[um]"), AutoSizeMode = DataGridViewAutoSizeColumnMode.AllCells });
        }
        ...
        public override void BuildRowData(DataGridViewRow dataGridViewRow, Production production)
        {
            ...
            if (!float.IsNaN(production.Value))
                dataGridViewRow.Cells[4].Value = production.Value;
        }
    }
}
